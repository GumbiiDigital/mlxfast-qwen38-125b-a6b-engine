...
The ranked run measures a pool of 8 prompts, ONE STREAM AT A TIME (David
ruling 2026-08-27: "Single-stream only"). The ranked run measures your engine
and a serial control engine in the same session. The score compares the two.
...
candidate leg, over the same 8 prompts.

The MTP arm and the serial control are both measured single-stream. That paired
comparison is the scored value.
...
### One resident worker per measured window

The ranked run measures inside ONE window, and the weights load ONCE in it.

The benchmarker starts `bench-worker runtime-worker` once for each phase: the
warmup, the timed prefill, the timed decode and the correctness pass. Each
start used to load the whole checkpoint again. It no longer does.

`tools/qwen38-125b-a6b-measure-and-score.sh` opens the window itself. It takes
the box GPU lock `/tmp/mtplx-gpu-exclusive.lock` and holds it for the whole
measurement. Inside that lock it runs `tools/resident-up.sh`, which starts ONE
`bench-worker resident`, waits for it to answer, and exports its socket path as
`BENCH_WORKER_RESIDENT_SOCKET`. Each per-phase worker connects to that socket
and loads nothing. The wrapper halts the resident when the window ends, on
success and on failure.

| Name | What it is |
|---|---|
| `BENCH_WORKER_RESIDENT_SOCKET` | The resident's Unix socket. A per-phase worker attaches to it instead of loading. |
| `RESIDENT_IDENTITY_FILE` | A JSON file that records the resident, its socket and its hello. |
...
`MLXFAST_RESIDENT_WORKER=0` is the only opt-out, and only that exact value. It
restores the earlier behaviour: no lock is taken here, no resident starts, and
each phase loads the weights again.
...
3. A runner advertises the ranked label set
   ...
   against the fixture pins before any measurement.
4. Scoring is single-stream. `scored_batch_size` is `1`. The timed leg runs
   one stream with the MTP head at the declared depth and scores against the
   golden's own baseline pair.
