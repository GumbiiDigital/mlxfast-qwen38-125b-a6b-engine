...
/// The Qwen 3.8 125B A6B TARGET's frozen quantization geometry.
///
...
/// the per-tensor override table (empty on this target), and nothing ever read back the
...
/// `Qwen4ExpRuntimeWeights.quantizeUniformly`, which lives in
...
/// (`Qwen4ExpEngineConfig`) and `Sources/MLXFastTransform` (`PinnedGeometry`) are
...
public struct Gemma4TargetQuantizationPins: Equatable, Sendable {
    ...
    /// Bit width the promoted projection families carry, on every layer. On a
    /// target that promotes nothing this equals `fallbackBits`.
    ...
    /// (`Sources/MLXFastTransform/Qwen4ExpCheckpointValidation.swift`'s
    ...
    /// The projection families promoted to `overrideBits` on every layer.
    /// EMPTY on a target that promotes nothing.
    ...
    /// The shipped target's geometry.
    ///
    /// UNIFORM. This checkpoint promotes nothing: every quantized module is
    /// affine, 4 bits, group size 32. `overrideBits` therefore equals
    /// `fallbackBits` and `overrideFamilies` is empty, which makes
    /// `runtimeOverridePaths` empty and turns the override half of the gate
    /// into a statement that NOTHING may carry a different width. That is a
    /// stronger check than the promoted-family one it replaces, not a weaker
    /// one: on the previous target only four families were pinned, and here
    /// every module is.
    ...
    /// `Qwen4ExpWeightNames.modelPrefix` (`language_model.model`) with the
