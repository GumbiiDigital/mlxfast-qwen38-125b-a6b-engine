// bench-worker — copied from the pinned fork at commit 449f2d01,
// Executables/bench-worker/main.swift, and kept a SHIM the same way: argument
// parsing lives in MLXRunners/BenchWorkerLaunch.swift and the protocol loop in
// MLXRunners/BenchWorker.swift.
//
// THE ONE DIFFERENCE from the fork's shim is step 1b below: this repository's
// own Runner — Runner/Qwen4ExpRunner.swift, an editable path — is registered
// before anything resolves a runner, so it SHADOWS the fork's built-in
// Qwen4ExpRunner on the same `model_type`. That is what makes the model
// family's code editable in this repository while the engine core stays a
// pinned submodule.
...
func probeDevice() -> String {
        ...
        // `[UInt8]` + `String(decoding:as:)`, not the fork's `[CChar]` +
        // `String(cString:)`: the latter is deprecated in Swift 6.3, and this
        // repository's warning gate (tools/ci-swift-warning-gate.sh) makes a
        // warning under Sources/ an error. Same bytes, same string -- sysctl
        // writes a NUL-terminated brand string and the prefix stops at it.
        var buffer = [UInt8](repeating: 0, count: size)
        guard sysctlbyname("machdep.cpu.brand_string", &buffer, &size, nil, 0) == 0 else {
            return "unknown"
        }
        let brand = String(decoding: buffer.prefix { $0 != 0 }, as: UTF8.self).lowercased()
...
func failWithUsage(_ error: some Error, topic: BenchWorkerUsage.Topic) -> Never {
    // `String(describing:)`, not the fork's `(error as? CustomStringConvertible)?
    // .description ?? "\(error)"`. Same string for both cases -- it prefers
    // `CustomStringConvertible.description` and otherwise renders the value the
    // way interpolation does -- but the conditional cast always succeeds on a
    // `some Error`, and Swift 6.3 warns about it. This repository's warning gate
    // (tools/ci-swift-warning-gate.sh) makes a warning under Sources/ an error,
    // which is why the fork's spelling cannot ride here unchanged.
...
// 1b. THE TRACK RUNNER, before anything resolves one. A later registration
//     replaces an earlier claim on the same `model_type`
//     (MLXRunners/RunnerRegistry.swift), so this repository's editable Runner
//     shadows the fork's built-in one for `qwen4_exp` and `qwen4_exp_text` —
//     for `manifest`, for `runtime-worker` and for `resident` alike.
RunnerRegistry.shared.register(TrackQwen4ExpRunner.self)
