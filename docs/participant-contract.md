...
The timed prompt pool is NOT armed. All 8 `timed_prompt_pool[]` entries carry
the exact pending sentinel `QWEN38-125B-A6B-MLX-PENDING-ORGANIZER` in
`r2_path` and `sha256`, with a byte count of 0.
`hidden_correctness_golden` carries the same sentinel. No ORGANIZER goldens
exist for this track -- the public goldens in `correctness_prompts/` are a
different thing and do exist (section 11.3). The sentinel is matched exactly;
it is never a prefix test.
...
| `timed_prompt_pool[]`, 8 tapes | Not authored yet. The 8 slots carry the pending sentinel. | **No.** When they exist they live in R2, the GETs are credentialed, and the objects are benchd tape documents rather than `--golden` files. |
| `hidden_correctness_golden` | Not authored yet. The slot carries the pending sentinel. | **No.** It is the token-fidelity oracle. |
...
`benchmarkCommand` targets `benchd measure-job` through
...
```text
benchd measure-job --contract fixtures/qwen3_8_125b_a6b_track.json \
  --candidate . --baseline $MLXFAST_QWEN38_BASELINE_WORKSPACE \
  --golden <each *.json in $MLXFAST_QWEN38_GOLDEN_DIR> \
  --min-pairs 4 --target-pairs 4 \
  --tag <run tag> --out <results dir>
```

The script then reads the composite off `results.json` and converts it to the
`{score, metrics}` shape the scorer requires.
...
**ONE RESIDENT WORKER PER WINDOW.** The measured topology is one GPU window
with one weight load in it. `tools/qwen38-125b-a6b-measure-and-score.sh` takes
the box GPU lock `/tmp/mtplx-gpu-exclusive.lock`, holds it for the whole
measurement, and runs the measurement under `tools/resident-up.sh`. That
wrapper starts ONE `bench-worker resident` over the transformed weights and the
n-gram shard directory the fixture names, and exports its socket as
`BENCH_WORKER_RESIDENT_SOCKET`. Every per-phase `bench-worker runtime-worker`
the benchmarker starts attaches to that socket instead of loading the
checkpoint again, and the wrapper halts the resident when the window ends. The
benchmarker forwards the socket name only from a channel that carries mlxfast-bench
#271; an older channel binary removes it, the worker then serves in its own
process, and the run measures exactly what it measured before. Nothing about
this changes what is scored: it removes repeated loads, not measured work. The
opt-out is `MLXFAST_RESIDENT_WORKER=0`, and only that exact value.
