...
**WHERE THE WRAP IS.** In `tools/qwen38-125b-a6b-measure-and-score.sh`, not at
a call site. The script resolves the pinned benchd, the arm gate, the live
golden and its pin, the engine binary, the weights and the n-gram shards
FIRST -- all of that loads nothing -- and only then opens the window:

```text
<GPU lock holder>
  -> tools/resident-up.sh --weights <weights> --ngram <ngram shards>
     -- ./tools/qwen38-125b-a6b-measure-and-score.sh
        -> benchd iterate --mode official
           -> bench-worker runtime-worker   (attaches, loads nothing)
```

The script re-runs itself inside the window. The inner run sees
`BENCH_WORKER_RESIDENT_SOCKET` already set and measures instead of wrapping
again, so there can be no second resident. The wrap sits in the script so that
EVERY entry point gets it: `benchmark.json` `benchmarkCommand`, the ranked
workflow step in `.github/workflows/benchmark.yml`, and a hand invocation on
the box.

**THE GPU LOCK.** A resident holds about 113 GB of unified memory, so the
window must be exclusive. The measure script takes
`/tmp/mtplx-gpu-exclusive.lock` FIRST and holds it for the whole measurement:
the lock holder is the outermost process and becomes the wrapper through
...
the window -- and it REFUSES to start a resident when nobody holds it. A wait
of `MLXFAST_GPU_LOCK_TIMEOUT_S` seconds (1800 by default) ends in a refusal,
never in a measurement beside another owner.

**THE NAMES.**

| Name | What it is |
|---|---|
| `BENCH_WORKER_RESIDENT_SOCKET` | The resident's Unix socket, exported into the wrapped command. |
| `RESIDENT_IDENTITY_FILE` | JSON: the resident, its pid, its socket, its hello. |
...
| `MLXFAST_RESIDENT_WORKER` | The opt-out. `0`, and only `0`, measures without a resident. |
...
is the resident's own load. So this wiring is safe to land before that change
reaches the channel, and it starts saving loads on the day it does.

**THE OPT-OUT.** `MLXFAST_RESIDENT_WORKER=0` restores the earlier path exactly:
no lock is taken by the script, no resident starts, and benchd starts per-phase
workers that each load. It exists for a box where the resident is the thing
under test, and for a bisect.
