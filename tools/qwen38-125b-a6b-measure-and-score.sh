...
# preSubmitCommand entry point for track qwen3.8-125b-a6b-mlx-v1.
#
...
#
# ONE RESIDENT bench-worker PER WINDOW (David 2026-08-30: weights load ONCE).
# benchd spawns `bench-worker runtime-worker` once per phase -- warmup, timed
# prefill, timed decode, correctness -- and an in-process spawn loads the whole
# checkpoint each time. So the weights get an OWNER for the window: this script
# re-runs itself under tools/resident-up.sh, which boots exactly ONE
# `bench-worker resident`, exports BENCH_WORKER_RESIDENT_SOCKET, and halts the
# resident when the window ends. Each per-phase worker attaches to that socket
# and loads nothing.
#
# THIS SCRIPT OWNS THE GPU WINDOW. A resident holds ~113 GB of unified memory,
# so it must not outlive the measurement and must not share the box with
# another loader. This script therefore takes the fleet GPU lock
# (/tmp/mtplx-gpu-exclusive.lock) FIRST, and holds it for the whole window: the
# lock holder is the outermost process, resident-up.sh runs inside it, and
# resident-up.sh REFUSES to boot when nobody holds that lock. resident-up.sh
# never takes the lock itself, because a lock it took would end when it exits,
# which is not the window.
#
# THE RE-RUN IS THE WRAP. The effective process tree of a ranked run is
#   <gpu lock holder> -> tools/resident-up.sh --weights <dir> --ngram <dir>
#                        -- ./tools/qwen38-125b-a6b-measure-and-score.sh
# and the inner run sees BENCH_WORKER_RESIDENT_SOCKET already set, so it does
# the measurement instead of wrapping again. The wrap lives here rather than at
# one call site so that EVERY entry point gets it: benchmark.json's
...
# safe before that change lands: it costs one extra load (the resident's) and
# changes no measured value.
#
# THE OPT-OUT is MLXFAST_RESIDENT_WORKER=0, and only that literal value. It
# restores the pre-resident path byte for byte: no lock is taken here, no
# resident is booted, and benchd spawns per-phase workers that each load.
#
...
#
# The design rationale lives in docs/qwen38-125b-a6b-port-notes.md section 9 and
# fixtures/qwen3_8_125b_a6b_track.json scoring_semantics.
#
# Usage:
...
#   MLXFAST_RESIDENT_WORKER          Whether the measurement runs under ONE
#                                     resident bench-worker. Default: on. The
#                                     ONLY opt-out is the explicit value 0,
#                                     which runs the measurement exactly as it
#                                     ran before this wiring (no resident, no
#                                     GPU lock taken here, one weight load per
#                                     spawned phase).
#   RESIDENT_UP_LOCK_PATH            The GPU lock this script holds for the
#                                     whole resident window. Default:
...
# maps "mtpN" to the pinned botany.mtpN.golden.json. A declared depth with no
...
if [[ "${SPEC_DESC}" != "serial" ]]; then
  ...
  if [[ -z "${LIVE_GOLDEN_BASENAME}" || "${LIVE_GOLDEN_BASENAME}" != *.golden.json ]]; then
    echo "qwen38-125b-a6b-measure-and-score.sh: live_golden_speculative['${SPEC_DESC}'] names no *.golden.json (r2_path='${LIVE_GOLDEN_BASENAME}')." >&2
    exit 1
  fi
  echo "qwen38-125b-a6b-measure-and-score.sh: declared spec ${SPEC_DESC}; timed oracle is ${LIVE_GOLDEN_BASENAME}" >&2
fi
...
qwen38-125b-a6b-measure-and-score.sh: MLXFAST_QWEN38_GOLDEN_DIR is unset or missing.
  ...
  (docs/qwen38-125b-a6b-port-notes.md section 5.3) and this job holds no credential
...
if [[ ! -d "${BASELINE_WORKSPACE}" ]]; then
  echo "qwen38-125b-a6b-measure-and-score.sh: MLXFAST_BASELINE_WORKSPACE is not a directory: ${BASELINE_WORKSPACE}" >&2
  exit 1
fi
...
if [[ ! -f "${BASELINE_CALIBRATION}" ]]; then
  echo "qwen38-125b-a6b-measure-and-score.sh: MLXFAST_BASELINE_CALIBRATION is not a file: ${BASELINE_CALIBRATION}" >&2
  exit 1
fi
...
case "${ENGINE_BIN_ABS}" in
  "${SCRIPT_DIR_ABS}"/*)
    ...
    ;;
  *)
    echo "qwen38-125b-a6b-measure-and-score.sh: the engine at ${ENGINE_BIN_ABS} is outside this checkout (${SCRIPT_DIR_ABS})." >&2
...
if [[ ! -d "${WEIGHTS_PATH}" ]]; then
  echo "qwen38-125b-a6b-measure-and-score.sh: the transformed weights directory is missing: ${WEIGHTS_PATH}" >&2
...
case "${WEIGHTS_PATH_ABS}" in
  "${SCRIPT_DIR_ABS}"/*)
    ...
    ;;
  *)
    echo "qwen38-125b-a6b-measure-and-score.sh: the transformed weights at ${WEIGHTS_PATH_ABS} are outside this checkout (${SCRIPT_DIR_ABS})." >&2
...
# ---------------------------------------------------------------------------
# THE N-GRAM ROW SOURCE. The 29.8 GiB n-gram / PLE table is never model
# parameters: the fork's Qwen4ExpRunner builds a disk-resident row source from
# the `qwen4exp.ngramRowSource` resource, whose value is the DIRECTORY that
# holds the n-gram shard files the offline transform wrote. Without it the
# runner refuses at load, by name.
#
# The directory is read FROM the fixture (`ngram_shard_dir`), never from a
# submission-editable path, and is resolved against the workspace root when it
# is relative. benchd forwards it to the engine as
# `--resource qwen4exp.ngramRowSource=<dir>` on the spawn argv.
NGRAM_SHARD_DIR="$(jq -r '.ngram_shard_dir // empty' "${CONTRACT}")"
if [[ -z "${NGRAM_SHARD_DIR}" ]]; then
  echo "qwen38-125b-a6b-measure-and-score.sh: fixture declares no ngram_shard_dir; the engine cannot resolve the n-gram row source." >&2
  exit 1
fi
if [[ "${NGRAM_SHARD_DIR}" != /* ]]; then
  NGRAM_SHARD_DIR="${SCRIPT_DIR}/${NGRAM_SHARD_DIR}"
fi

# ---------------------------------------------------------------------------
# THE RESIDENT WINDOW. Everything above resolved WITHOUT loading anything: the
# pinned benchd, the arm gate, the golden and its pin, the engine binary, the
# weights and the n-gram shards. This is the last point before benchd starts
# spawning workers, so this is where the window opens.
#
# Two conditions, and both are checked here rather than deeper:
#   * MLXFAST_RESIDENT_WORKER=0 -- the explicit opt-out; run as before.
#   * BENCH_WORKER_RESIDENT_SOCKET already set -- this IS the inner run, i.e.
#     resident-up.sh has already booted the resident and re-entered this
#     script. Wrapping again would boot a second resident, which is exactly
#     what resident-up.sh refuses.
RESIDENT_WORKER="${MLXFAST_RESIDENT_WORKER:-1}"
if [[ "${RESIDENT_WORKER}" != "0" && -z "${BENCH_WORKER_RESIDENT_SOCKET:-}" ]]; then
  RESIDENT_UP="${SCRIPT_DIR}/tools/resident-up.sh"
  if [[ ! -x "${RESIDENT_UP}" ]]; then
    echo "qwen38-125b-a6b-measure-and-score.sh: the resident wrapper is missing or not executable: ${RESIDENT_UP}" >&2
    echo "  the ranked path runs the measurement under ONE resident bench-worker; set MLXFAST_RESIDENT_WORKER=0 to measure without it (one weight load per phase)." >&2
    exit 1
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    echo "qwen38-125b-a6b-measure-and-score.sh: python3 is required to hold the GPU lock for the resident window." >&2
    exit 1
  fi
  ...
  if ! printf '%s' "${GPU_LOCK_TIMEOUT_S}" | grep -Eq '^[1-9][0-9]*$'; then
    echo "qwen38-125b-a6b-measure-and-score.sh: MLXFAST_GPU_LOCK_TIMEOUT_S must be a positive integer (got '${GPU_LOCK_TIMEOUT_S}')." >&2
    exit 1
  fi
  # The resident and every attaching phase must run ONE binary, so the engine
  # this script resolved is exported rather than re-derived by the wrapper.
  export MLXFAST_ENGINE_BIN="${ENGINE_BIN}"
  export RESIDENT_UP_LOCK_PATH="${GPU_LOCK_PATH}"
  echo "qwen38-125b-a6b-measure-and-score.sh: taking the GPU lock ${GPU_LOCK_PATH} for the whole measurement, then booting ONE resident bench-worker (weights load once). Opt out with MLXFAST_RESIDENT_WORKER=0." >&2
  # The lock holder is the OUTERMOST process of the window: it holds an
  # exclusive flock on an inheritable descriptor and then becomes the wrapper
  # through execv, so the lock lives exactly as long as the measurement. A
...
def on_timeout(*_):
    sys.stderr.write(
        \"qwen38-125b-a6b-measure-and-score.sh: another owner has held \"
        + lock_path
...
" "${GPU_LOCK_PATH}" "${GPU_LOCK_TIMEOUT_S}" \
    ...
    --ngram "${NGRAM_SHARD_DIR}" \
    -- \
    "${SCRIPT_DIR}/tools/qwen38-125b-a6b-measure-and-score.sh" "$@"
fi

# THE FLAG IS CONDITIONAL, and the condition is the installed benchd. benchd
# builds the engine spawn argv itself and fences it against the verb's accepted
# option surface, so a channel binary with no `--engine-resource` would refuse
# the run rather than forward the resource. Probe its usage text and pass the
# flag only when it is there; when it is not, the engine is spawned without the
# resource and the runner's own refusal names what is missing.
ENGINE_RESOURCE_ARGS=()
if "${BENCHD}" iterate --help 2>&1 | grep -q -- '--engine-resource'; then
  ENGINE_RESOURCE_ARGS=(--engine-resource "qwen4exp.ngramRowSource=${NGRAM_SHARD_DIR}")
else
  echo "qwen38-125b-a6b-measure-and-score.sh: this benchd has no --engine-resource; the engine is spawned without the n-gram row source and the runner will refuse at load." >&2
fi
...
# THE DECLARED DRAFT DEPTH. tools/spec-declaration.sh is the single trusted
# reader of the participant's mtp-head.manifest.json `spec` block; it refuses
# (exit 1, before any engine spawns) on a malformed or out-of-envelope
# declaration. A serial declaration (absent, disabled, or 0) sends NO spec, and
# the engine runs depth 0 -- the baseline validation's leg. A depth N sends
# `--mtp-depth N`; the engine echoes effective_spec and the seal carries it.
...
if [[ "${DECLARED_DEPTH}" != "0" ]]; then
  if "${BENCHD}" iterate --help 2>&1 | grep -q -- '--mtp-depth'; then
    ...
    echo "qwen38-125b-a6b-measure-and-score.sh: the declaration requests MTP depth ${DECLARED_DEPTH}; benchd sends it on the wire and the engine's effective_spec echo is sealed." >&2
  else
    echo "qwen38-125b-a6b-measure-and-score.sh: REFUSING -- the declaration requests MTP depth ${DECLARED_DEPTH} but this benchd has no --mtp-depth; the leg would run serial against a speculative declaration." >&2
    exit 1
  fi
else
  echo "qwen38-125b-a6b-measure-and-score.sh: the declaration is serial; benchd sends no spec (depth 0)." >&2
fi
...
if [[ -z "${RUNNER_NAME:-}" ]]; then
  ...
  if [[ -n "${CALIBRATION_BOX}" ]]; then
    ...
    echo "qwen38-125b-a6b-measure-and-score.sh: RUNNER_NAME is unset (hand run); naming the box '${CALIBRATION_BOX}' from the calibration file, so benchd matches the band to the machine it was measured on." >&2
  else
    echo "qwen38-125b-a6b-measure-and-score.sh: RUNNER_NAME is unset and the calibration file names no box; benchd resolves the box identity itself." >&2
  fi
fi
