...
#
# THIS SCRIPT STILL OWNS THE GPU WINDOW. A resident holds ~113 GB of unified
...
#
# WHAT THIS SCRIPT STILL OWNS IS THE LOCK. A resident holds ~113 GB of unified
...
if [[ -n "${BENCH_WORKER_RESIDENT_SOCKET:-}" ]]; then
  echo "qwen38-125b-a6b-measure-and-score.sh: BENCH_WORKER_RESIDENT_SOCKET is set (${BENCH_WORKER_RESIDENT_SOCKET})." >&2
...
if [[ "${MLXFAST_GPU_WINDOW_HELD:-0}" != "1" ]]; then
  if ! command -v python3 >/dev/null 2>&1; then
    echo "qwen38-125b-a6b-measure-and-score.sh: python3 is required to hold the GPU lock for the measurement window." >&2
    exit 1
  fi
  ...
  echo "qwen38-125b-a6b-measure-and-score.sh: taking the GPU lock ${GPU_LOCK_PATH} for the whole measurement. benchd boots ONE resident per leg inside it (tools/resident-up.sh --boot), from that leg's own tree." >&2
