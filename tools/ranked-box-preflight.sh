...
#
# SINGLE-LEG, NOT PAIRED (David 2026-08-27). This track scores ONE leg -- the
# submission's declared spec, serial at the MTP-0 launch reference -- over the
# ONE live golden, against the OFFICIAL_BASELINE constants benchd resolves from
# the track id. There is NO separate baseline workspace to stage and no paired
# control run: `benchd iterate --mode official` seals the composite itself
# (tools/qwen38-125b-a6b-measure-and-score.sh). So this preflight verifies the
# staged pool tapes (and the live golden among them) and the fixture arm state --
# it does NOT read a MLXFAST_QWEN38_BASELINE_WORKSPACE.
#
...
for var in BENCHD BENCHCTL MLXFAST_SKIP_WEIGHTS_DOWNLOAD SKIP_MODEL_DOWNLOAD MLXFAST_LOCAL_COOL_GATE MLXFAST_LOCAL_ALLOW_GOLDEN_DRIFT MLXFAST_GPU_TEMP_CMD; do
  eval "value=\${${var}:-}"
