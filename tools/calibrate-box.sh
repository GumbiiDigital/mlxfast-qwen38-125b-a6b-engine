...
# --passes times under the FULL official methodology -- the cool gate per pass,
...
# measurement in tools/qwen38-125b-a6b-measure-and-score.sh. A calibration
...
# The n-gram row source, read FROM the fixture and resolved against the
# WORKSPACE: the reference runner refuses at load without it, by name.
NGRAM_SHARD_DIR="$(jq -r '.ngram_shard_dir // empty' "${CONTRACT}")"
[[ -n "${NGRAM_SHARD_DIR}" ]] || die "the fixture declares no ngram_shard_dir"
if [[ "${NGRAM_SHARD_DIR}" != /* ]]; then
  NGRAM_SHARD_DIR="${BASELINE_WORKSPACE}/${NGRAM_SHARD_DIR}"
fi
...
echo "calibrate-box.sh: taking the GPU lock ${GPU_LOCK_PATH}, then calibrating ${PASSES} pass(es) of the serial-control leg on ${BASELINE_WORKSPACE} over ${LIVE_GOLDEN_NAME}" >&2
...
" "${GPU_LOCK_PATH}" "${GPU_LOCK_TIMEOUT_S}" \
  ...
  --engine-resource "qwen4exp.ngramRowSource=${NGRAM_SHARD_DIR}" \
