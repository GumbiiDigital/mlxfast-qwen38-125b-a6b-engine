...
# is a box-only script -- the real one loads a 113 GB checkpoint -- so what a
...
LIVE_GOLDEN_NAME="$(jq -r '.live_golden' "${ROOT}/fixtures/qwen3_8_125b_a6b_track.json")"
...
printf '{"branch":"qwen3.8-125b-a6b-v1","source_commit":"%s","sha256":"%s","bytes":1}\n' \
...
run_calibrate() {
  ...
  [[ $# -eq 0 ]] || shift
  env -u MLXFAST_QWEN_MTP_TRACK_ID -u RUNNER_NAME \
...
if [[ "${rc}" -ne 0 ]]; then
  ...
  argv_has_pair "${WORK}/case1.argv" --engine-resource "qwen4exp.ngramRowSource=${REF_WS}/weights" \
    || fail "case 1: argv does not point the n-gram row source at the reference tree"
...
if [[ "${failures}" -eq 0 ]]; then
  echo "test-calibrate-box.sh: all 11 cases passed"
  exit 0
fi
