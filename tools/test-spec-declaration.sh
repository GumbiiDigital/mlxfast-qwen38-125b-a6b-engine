...
# draft depth becomes benchd's --mtp-depth request, and this script is the one
# trusted bridge that turns the participant's declaration into that request --
# so an enabled depth outside the contract's permitted_draft_depths must be
# REFUSED here, before any engine spawns. The accept cases pin 1/2/3; the refuse
