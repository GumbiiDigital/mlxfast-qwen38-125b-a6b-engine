...
# the contract fixture, the runner label, the gitlink and the channel all name
...
SEED_GITLINK="9999999999999999999999999999999999999999"
...
#
#   docs/qwen38-125b-a6b-port-notes.md   the source track's porting history and
...
# tree. git archive omits the submodule (a gitlink is not a file), so the
# gitlink is re-created explicitly: the stamp must MOVE it, and a tree without
# one would let that assertion pass vacuously.
seed() {
  local dir="$1"
  mkdir -p "${dir}"
  ...
  git -C "${dir}" init -q
  git -C "${dir}" add -A
  git -C "${dir}" update-index --add --cacheinfo "160000,${SEED_GITLINK},Vendor/mlx-swift-lm"
...
# the submodule pin
gitlink="$(git -C "${SEED}" ls-files --stage Vendor/mlx-swift-lm | awk '{print $2}')"
if [[ "${gitlink}" == "${FORK_SHA}" ]]; then
  pass "the Vendor/mlx-swift-lm gitlink is the fork sha"
else
