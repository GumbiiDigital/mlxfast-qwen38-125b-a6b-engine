...
REFERENCE_COMMIT="$(jq -r '.baseline_reference_commit' "${REPO_ROOT}/fixtures/qwen3_8_125b_a6b_track.json")"
...
run_stage() {
  ...
  env -u MLXFAST_BASELINE_SOURCE -u MLXFAST_FORK_MIRROR -u MLXFAST_METALLIB_STAGE \
...
if [[ "${failures}" -eq 0 ]]; then
  echo "test-stage-baseline-workspace.sh: all 8 cases passed"
  exit 0
fi
