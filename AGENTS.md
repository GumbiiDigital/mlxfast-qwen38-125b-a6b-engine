...
golden, and pins one per-depth oracle for each draft depth 1 to 6. The goldens
in `correctness_prompts/` were recorded on the ranked box from the pinned Qwen
checkpoint. Each golden carries its own baseline pair, measured by the official
ranked path.
...
the goldens are staged on the box and pin-verified by
...
Scoring is single-stream. The fixture sets `scored_batch_size` to `1`. The
timed leg runs one stream with the MTP head at the declared depth. The batched
cohort path is not part of this track.
